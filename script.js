...
async function processImage(image) {
    ...
    try {
        ...
        // Preprocesar correctamente la imagen
        const imgTensor = tf.tidy(() => {
            return tf.browser.fromPixels(image)
                .resizeNearestNeighbor([224, 224])  // redimensionar
                .toFloat()
                .div(tf.scalar(255))                // normalizar entre 0–1
                .expandDims();                      // [1, 224, 224, 3]
        });
        
        // Hacer predicción (cuidado con tf.GraphModel)
        ...
        // Liberar memoria (tf.tidy ya limpia, pero aseguramos)
        tf.dispose([imgTensor, prediction]);
        
        //Determinar la clase correcta
        const resultIndex = predictionData.indexOf(Math.max(...predictionData));
        displayPrediction(resultIndex === 1 ? predictionData[1] : predictionData[0]);
